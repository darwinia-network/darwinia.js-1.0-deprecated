...
function unwrapSessionIds (stashId: AccountId, queuedKeys: [AccountId, Keys][], nextKeys: Option<Keys>): { nextSessionIds: AccountId[]; sessionIds: AccountId[] } {
  const sessionIds = (queuedKeys.find(([currentId]): boolean => currentId.eq(stashId)) || [undefined, [] as AccountId[]])[1];
...
function retrieveCurr (api: ApiInterfaceRx, stashId: AccountId): Observable<MultiResult> {
  return api.derive.session.indexes().pipe(switchMap(({ activeEra }) => api.queryMulti<MultiResult>([[api.query.staking.bonded, stashId], [api.query.staking.nominators, stashId], [api.query.staking.payee, stashId], [api.query.staking.validators, stashId], api.consts.session?.dedupKeyPrefix ? [api.query.session.nextKeys, [api.consts.session.dedupKeyPrefix, stashId]] : [api.query.session.nextKeys, stashId], [api.query.staking.erasStakers, [activeEra, stashId]]])));
}
...
function retrieveController (api: ApiInterfaceRx, stashId: AccountId, queuedKeys: Vec<ITuple<[AccountId, Keys]>>, [controllerIdOpt, nominatorsOpt, rewardDestination, validatorPrefs, nextKeys, exposure]: MultiResult): Observable<DeriveStakingQuery> {
  ...
  return controllerId
    ? api.query.staking.ledger<Option<DarwiniaStakingStructsStakingLedger>>(controllerId).pipe(
      map(
        (stakingLedgerOpt: Option<DarwiniaStakingStructsStakingLedger>): DeriveStakingQuery => ({
          accountId: stashId,
          controllerId,
          exposure,
...
export function queryWithQueued (instanceId: string, api: ApiInterfaceRx): (accountId: Uint8Array | string, queuedKeys: Vec<ITuple<[AccountId, Keys]>>) => Observable<DeriveStakingQuery> {
  return memo(instanceId, (accountId: Uint8Array | string, queuedKeys: Vec<ITuple<[AccountId, Keys]>>): Observable<DeriveStakingQuery> => {
    const stashId: AccountId = api.registry.createType('AccountId', accountId);

    return retrieveCurr(api, stashId).pipe(switchMap((result): Observable<DeriveStakingQuery> => retrieveController(api, stashId, queuedKeys, result)));
  });
}
